...
# Sample the frequency and time data to reduce resolution
frequency_sample_rate = 60  # Take every 50th frequency channel
time_sample_rate = 2       # Take every 60th time point

sampled_frequencies = data.index[::frequency_sample_rate]
sampled_time_stamps = time_stamps[::time_sample_rate]
sampled_data = data.iloc[::frequency_sample_rate, ::time_sample_rate]

# Bin the frequency channels into smaller ranges
bin_size = 2500  # Number of sampled frequency channels per bin
num_bins = int(np.ceil(len(sampled_frequencies) / bin_size))
bins = [sampled_frequencies[i*bin_size:(i+1)*bin_size] for i in range(num_bins)]
...
def update_graph(selected_bin, n):
    ...
    F, T = np.meshgrid(bin_range, sampled_time_stamps)
    power_readings = sampled_data.loc[bin_range].values.T
    
    # Create the contour plot for the current bin
    fig = go.Figure(data=go.Contour(
        z=power_readings,
        x=bin_range,
        y=sampled_time_stamps,
        colorscale='Cividis',
        contours=dict(
            start=-110,
            end=-20,
        ...
        line=dict(
            width=0.1  # Adjust the line width here for thinner contour lines
        )
    ))
