...
# Calculate the IQR for all data points
all_power_readings = power_readings.flatten()
Q1 = np.percentile(all_power_readings, 25)
Q3 = np.percentile(all_power_readings, 75)
IQR = Q3 - Q1

# Define the cutoff as Q3 + 1.5 * IQR
cutoff = Q3 + 1.5 * IQR

# Set values below the cutoff to -90 dBm for Anritsu
filtered_power_readings = np.where(power_readings < cutoff, -90, power_readings)

# Set values below the cutoff to -110 dBm for Signal Hounds
